...
static pci_device_t pcidevs[MALTA_PCI_DEVICE_COUNT];
static unsigned npcidev;
...
static void pci_bus_enumerate(pci_bus_device_t *pcib) {
  for (unsigned j = 0; j < 32; j++) {
    for (unsigned k = 0; k < 8; k++) {
      ...
      pci_device_t *pcidev = &pcidevs[npcidev++];
      pcidev->bus = pcib->bus;
...
static void pci_bus_assign_space(pci_bus_device_t *pcib) {
  ...
  printf("devs = %d, nbars = %d\n", ndevs, nbars);

  resource_t *bars[nbars];
  memset(bars, 0, sizeof bars);
  unsigned n = 0;

  TAILQ_FOREACH (pcidev, &pcib->devices, link) {
    for (unsigned i = 0; i < pcidev->nbars; i++)
      bars[n++] = &pcidev->bar[i];
  }
...
static void pci_bus_dump(pci_bus_device_t *pcib) {
  ...
  TAILQ_FOREACH (pcidev, &pcib->devices, link) {
    ...
    kprintf("[pci:%x:%x.%x]", pcidev->addr.bus,
    ...
    kprintf("%s", pci_class_code[pcidev->class_code]);

    if (vendor)
      kprintf(" %s", vendor->name);
    else
      kprintf(" vendor:$%x", pcidev->vendor_id);
    ...
    if (pcidev->pin)
      kprintf(" Interrupt: pin %c routed to IRQ %d\n",
    ...
    for (unsigned i = 0; i < pcidev->nbars; i++) {
      ...
      kprintf(" Region %d: %s at %p [size=$%x]\n", i, type,
